...
namespace Combat_Realism
{
    ...
    public class Gizmo_BackpackEquipment : Gizmo
    {
        ...
        private int iconsPerRow
        {
            get
            {
                if (backpack.stackCount <= 4)
                    return 2;
                else
                {
                    int count = Mathf.FloorToInt(backpack.stackCount - 4) / 2;
        ...
        public override GizmoResult GizmoOnGUI(Vector2 topLeft)
        {
            ...
            // Slots CompSlots
            {
                ...
                DrawSlots(wearer, backpack.TryGetComp<CompInventory>(), inventoryRect);
            }

            return new GizmoResult(GizmoState.Clear);
        }

        public void DrawSlots(Pawn wearer, CompInventory SlotsBackpackComp, Rect inventoryRect)
        {
            // draw text message if no contents inside
            if (SlotsBackpackComp.container.Count == 0)
            {
            ...
            else
            {
                ...
                for (int currentSlotInd = 0; currentSlotInd < iconsPerRow * numOfRow; currentSlotInd++)
                {
                    ...
                    if (currentSlotInd >= SlotsBackpackComp.container.Count)
                    {
                        slotRect.x = inventoryRect.x + inventoryRect.width / iconsPerRow * (currentSlotInd % iconsPerRow);
                        slotRect.y = inventoryRect.y + Height / 2 * (currentSlotInd / iconsPerRow);
                    ...
                    if (currentSlotInd < SlotsBackpackComp.container.Count)
                    {
                        ...
                        Thing currentThing = SlotsBackpackComp.container[currentSlotInd];
                        ...
                        {
                            this.thingIconSound = SoundDefOf.Click;
                            ...
                            if (Event.current.button == 0)
                            {
                                ...
                                if (currentThing != null && currentThing.def.equipmentType == EquipmentType.Primary && (currentThing.def.IsRangedWeapon || currentThing.def.IsMeleeWeapon))
                                {
                                    ...
                                    wearer.equipment.TryTransferEquipmentToContainer(wearer.equipment.Primary, wearer.inventory.container, out thingWithComps);
                                }
                                else
                                {
                                    this.backpack.numOfSavedItems--;
                                }
                            ...
                            else if (Event.current.button == 1)
                            {
                                List<FloatMenuOption> options = new List<FloatMenuOption>
                                {
                                    ...
                                    {
                                        Thing resultThing;
                                        SlotsBackpackComp.container.TryDrop(currentThing, wearer.Position,
                                ...
                                if (currentThing != null && currentThing.def.equipmentType == EquipmentType.Primary)
                                    ...
                                    {
                                        SlotsBackpackComp.TrySwitchToWeapon(currentThing as ThingWithComps);

                                    }));

                                // Apparel
                                else if (currentThing is Apparel)
                                {
                                    for (int j = wearer.apparel.WornApparel.Count - 1; j >= 0; j--)
                                    {
                                        Apparel apparel = wearer.apparel.WornApparel[j];
                                        if (!ApparelUtility.CanWearTogether(currentThing.def, apparel.def))
                                        {
                                            Apparel apparel2;
                                            wearer.apparel.TryDrop(apparel, out apparel2, wearer.Position, false);
                                            wearer.inventory.container.TryAdd(apparel2);
                                            this.backpack.numOfSavedItems++;
                                        }
                                    }
                                    wearer.apparel.Wear(currentThing as Apparel, true);
                                    wearer.inventory.container.Remove(currentThing as ThingWithComps);
                                    this.backpack.numOfSavedItems--;
                                }
                                ...
                                if (currentThing.def.thingCategories.Contains(DefDatabase<ThingCategoryDef>.GetNamed("FoodMeals")))
                                {
                                    ...
                                    {
                                        Thing dummy;
                                        SlotsBackpackComp.container.TryDrop(currentThing, wearer.Position, ThingPlaceMode.Direct, currentThing.def.ingestible.maxNumToIngestAtOnce, out dummy);
